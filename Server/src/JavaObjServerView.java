					...
					if (cm.getCode().matches("100")) { // 로그인 프로토콜
						...
						for (int i = 0; i < Room.size(); i++) {
							...
							if (port_number.equals(Integer.toString(rt.getServerPort()))) {
								if (rt.getUserCount() == 1)
									roomAccess = "true";
								else if (rt.getUserCount() == 0)
									roomAccess = "empty";
								else if (rt.getUserCount() >= 2)
									roomAccess = "full";
								else
									roomAccess = "Error";
							}
						...
						if (cnt == (Room.size() - 1))
							cm.setData(String.format("%d last %s %d", rt.getServerPort(), rt.getCreator(),
									rt.getUserCount()));
						else
							cm.setData(String.format("%d yet %s %d", rt.getServerPort(), rt.getCreator(),
									rt.getUserCount()));
						WriteOneObject(cm);
						cnt++;
						...
						try {
							...
							if (Room.size() > 0) {
								...
								emptyPortNumber = thisPortNumber + 1;
