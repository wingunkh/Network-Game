...
public class JavaObjClientGame extends JFrame {
	...
	private String backSrc = "src/images/background2.png";
	...
	private ImageIcon backIcon = new ImageIcon(
			new ImageIcon(backSrc).getImage().getScaledInstance(900, 600, java.awt.Image.SCALE_SMOOTH));
	...
	public JavaObjClientGame(String username, String ip_addr, String port_no) {
		...
		/* 베팅 버튼 */
		half = new JLabel(new ImageIcon(new ImageIcon("src/images/half2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
		half.setToolTipText("\"1.5\uB9CC\uC6D0\"");
		half.setBounds(686, 384, 130, 50);
		contentPane.add(half);

		ddadang = new JLabel(new ImageIcon(new ImageIcon("src/images/ddadang2.png").getImage().getScaledInstance(130,
				50, java.awt.Image.SCALE_SMOOTH)));
		ddadang.setToolTipText("\"2\uB9CC\uC6D0\"");
		ddadang.setBounds(524, 444, 130, 50);
		contentPane.add(ddadang);

		die = new JLabel(new ImageIcon(new ImageIcon("src/images/die2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
		die.setBounds(686, 444, 130, 50);
		contentPane.add(die);
		call = new JLabel(new ImageIcon(new ImageIcon("src/images/call2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
		call.setBounds(524, 504, 130, 50);
		contentPane.add(call);

		bbing = new JLabel(new ImageIcon(new ImageIcon("src/images/bbing2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
		...
		myJokbo.setFont(new Font("한컴 울주 반구대 암각화체", Font.PLAIN, 28));
		...
		/* 두루마리 */
		durumari = new JLabel(new ImageIcon(new ImageIcon("src/images/durumari.png").getImage().getScaledInstance(400,
				240, java.awt.Image.SCALE_SMOOTH)));
			...
			while (true) {
				try {
					...
					case "100": // 로그인 프로토콜
						uID = cm.getData();
						break;
					case "104": // 시작 버튼 생성 프로토콜 (방 인원 2명일 시)
					...
					case "3": // 베팅 시작 프로토콜
						if (uID.equals("1")) {
							ChatMsg chatmsg = new ChatMsg(UserName, "4", "베팅 시작!");
							SendObject(chatmsg);
						}
						break;
					case "4": // 베팅 활성화 프로토콜 (toggle)
						if (toggle) {
							if (uID.equals("1"))
								batting();
							else
					...
					case "5": // 베팅 진행 프로토콜
						...
						if (!previous.equals("none") && (!(cm.getData().split(" ")[0].equals(uID)))) {
							...
							if (previous.equals("bbing") && (property > 10000)) {
								...
								call.setIcon(new ImageIcon(new ImageIcon("src/images/call1.png").getImage()
										.getScaledInstance(130, 50, java.awt.Image.SCALE_SMOOTH)));
						...
						if (cm.getData().equals("1 bbing"))
							...
							bbing.setIcon(new ImageIcon(new ImageIcon("src/images/bbing2.png").getImage()
									.getScaledInstance(130, 50, java.awt.Image.SCALE_SMOOTH)));
							...
							half.setIcon(new ImageIcon(new ImageIcon("src/images/half2.png").getImage()
									.getScaledInstance(130, 50, java.awt.Image.SCALE_SMOOTH)));
			...
			for (int i = 0; i < 4; i++) {
				...
				case 2:
					...
					while (otherLeft.getX() > 100 || otherLeft.getY() > 30) {
						if (otherLeft.getX() > 100)
							otherLeft.setX(otherLeft.getX() - 5);
						if (otherLeft.getY() > 30)
				...
				case 3:
					...
					while (otherRight.getX() < 300 || otherRight.getY() > 30) {
						if (otherRight.getX() < 300)
							otherRight.setX(otherRight.getX() + 5);
						if (otherRight.getY() > 30)
	...
	public void Opening(String str) throws InterruptedException {
		...
		if (count == 2) {
			if (uID.equals("1")) {
				ChatMsg msg = new ChatMsg(UserName, "3", "베팅버튼 활성화");
				SendObject(msg);
			}
		}
	}
	...
	public void batting() {
		if (property > 10000) {
			...
			bbing.setIcon(new ImageIcon(new ImageIcon("src/images/bbing1.png").getImage().getScaledInstance(130, 50,
					java.awt.Image.SCALE_SMOOTH)));
		}
		if (property > 15000) {
			half.addMouseListener(halfpressed);
			half.setIcon(new ImageIcon(new ImageIcon("src/images/half1.png").getImage().getScaledInstance(130, 50,
					java.awt.Image.SCALE_SMOOTH)));
		}
		if (property > 20000) {
			ddadang.addMouseListener(ddadangpressed);
			ddadang.setIcon(new ImageIcon(new ImageIcon("src/images/ddadang1.png").getImage().getScaledInstance(130, 50,
					java.awt.Image.SCALE_SMOOTH)));
		}
		die.addMouseListener(diepressed);
		die.setIcon(new ImageIcon(new ImageIcon("src/images/die1.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
	}
	...
	public void waitbatting() {
		...
		bbing.setIcon(new ImageIcon(new ImageIcon("src/images/bbing2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
		half.setIcon(new ImageIcon(new ImageIcon("src/images/half2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
		ddadang.setIcon(new ImageIcon(new ImageIcon("src/images/ddadang2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
		die.setIcon(new ImageIcon(new ImageIcon("src/images/die2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
		call.setIcon(new ImageIcon(new ImageIcon("src/images/call2.png").getImage().getScaledInstance(130, 50,
				java.awt.Image.SCALE_SMOOTH)));
	}
	...
	public void updateMymoney(int updatedAmount) {
		if (updatedAmount < 10000) {
			...
			ddadang.removeMouseListener(ddadangpressed);
			ddadang.setIcon(new ImageIcon(new ImageIcon("src/images/ddadnag2.png").getImage().getScaledInstance(130, 50,
					java.awt.Image.SCALE_SMOOTH)));
			mymoney.setText(Integer.toString(updatedAmount));
		} else
			mymoney.setText(Integer.toString(updatedAmount));
	...
	public void battle() throws InterruptedException {
		if (uID.equals("1"))
			result = new JokboMatch(new Jokbo(myLeft, myRight).calculateJokbo() + " "
					+ new Jokbo(otherLeft, otherRight).calculateJokbo());
		else
			result = new JokboMatch(new Jokbo(otherLeft, otherRight).calculateJokbo() + " "
					+ new Jokbo(myLeft, myRight).calculateJokbo());
		updatePanmoney(0);
